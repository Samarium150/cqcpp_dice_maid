...
CQ_INIT {
...
#ifdef STD
            if (!CreateDirectory(path.c_str(), NULL)) throw - 1;
#endif
        } else if (path == "")
            throw - 1;
        Maid::init();
    ...
    on_private_message([](const PrivateMessageEvent &e) {
        ...
        try {
            ...
            */
            ...
            stringstream result;
            result << "私聊消息错误: " << to_string(err.code) << "\n原消息: " << msg;
            logging::error("私聊", result.str());
        }
    });
        ...
        */
        ...
        try {
            if (regex_match(msg, m_diss, dissmiss) && stoi(extract(msg, 1, ' ')) == get_login_user_id()) {
                Message message("");
                if (get_member_role(e.group_id, e.user_id) == GroupRole::MEMBER) {
            ...
            stringstream result;
            result << "群聊消息错误: " << to_string(err.code) << "\n原消息: " << msg;
            logging::error("群聊", result.str());
        } catch (const exception &exc) {
            stringstream result;
            result << "内部错误: " << exc.what() << "\n原消息: " << msg;
    ...
    */
    ...
    on_friend_request([](const FriendRequestEvent &e) { 
        set_friend_request(e.flag, RequestEvent::Operation::APPROVE);
    });

    on_group_request([](const GroupRequestEvent &e) {
